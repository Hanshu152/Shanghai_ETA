...
# 导出的特征数据集文件名
OUTPUT_FEATURE_FILE = "voyages_features_pure_sailing_v4.csv"
...
val_results_df['distance_bucket'] = pd.cut(val_results_df['dist_to_gate_km'], bins=bins, labels=labels)

bucket_metrics = val_results_df.groupby('distance_bucket', observed=False).agg(
    mae=('abs_error', 'mean'),
